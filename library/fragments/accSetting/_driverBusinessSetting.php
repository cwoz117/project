...
<div id="inputForm" class="right">
    <form id="basic_info" class="w3-container w3-padding" action="action/updateAccount.php" method="post">
        ...
        $sql = "SELECT wcb_no FROM Driver WHERE user_id = '$userID';";
        $result = $link->query($sql);
        $result = $result->fetch_assoc();
        $link->close();
        ?>
        <label for="wcb">WCB Number</label>
        <input type="text" id="wcb" name="wcb" placeholder=<?php echo $result["wcb_no"]?>>

        <?php
        require("global/db.php");
        $sql = "SELECT banking_info FROM Driver WHERE user_id = '$userID';";
        $result = $link->query($sql);
        $result = $result->fetch_assoc();
        $link->close();
        ?>
