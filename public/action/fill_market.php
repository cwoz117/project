...
function get_market_details(){
  ...
  $sql = "select * 
          from WorkOrder 
          where completed = 0 and workorder_no not in 
            (select workorder_no 
    ...
    $sql2 = "select * 
            ...
            where user_id = company_id and completed = false 
            except(
...
function print_rows($result, $link){
    ...
    while ($row = $result->fetch_assoc()){
        ...
        make_details($i, $row['pickup_address'],$row['dropoff_address'],$row['asset_value'],$row['cargo_type'],$row['gross_weight'],$row['contact_info'], $row['workorder_no'], $row['payload_id'], $row['user_id']);
        $i++;
    }
}
